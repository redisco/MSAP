...
import ua.com.msap.core.links.PI;
 ...
 * @author Neredko Mick
 */
public class PIReader extends LinkReader {

    @Override
      public Link parse(Element xmlElement,
            IVariablesStorage variablesStorage) throws Exception{
               if (xmlElement == null) {
            String message = "Попытка распознавания ПИ-звена из нулевого"
        ...
        if (kElement == null || kElement.getTextContent().isEmpty()) {
            String message = "У ПИ-звена не задан параметр 'K' (<K>...</K>).";
        ...
        if (tElement == null || tElement.getTextContent().isEmpty()) {
            String message = "У ПИ-звена не задан параметр 'T' (<T>...</T>).";
            throw new InvalidSchemeFormatException(message);
        }
